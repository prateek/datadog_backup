...
- [ ] The options `--ssh` and `--ssshh` are no longer supported. Instead, please use `--quiet` to suppress logging. `--debug` remains supported.
...
| DD_APP_KEY           | The Application key for the Datadog account                                    | none                      |
